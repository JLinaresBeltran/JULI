...
class SectorSpecificAgent extends Agent {
    ...
    async processComplaint(conversation, customerData) {
        try {
            logInfo('Iniciando procesamiento de queja', {
                sector: this.sector,
            ...
            const result = this.formatDocument(refinedDocument, customerData);

            // Notificar éxito
            await whatsappService.sendTextMessage(
                customerData.phone,
                "¡Listo! He terminado de procesar tu caso. El documento será enviado a tu correo electrónico."
            );

            logInfo('Queja procesada exitosamente', {
                sector: this.sector,
                customerId: customerData.phone
            });
...
class LegalAgentSystem {
    ...
    async processComplaint(sector, conversation, customerData) {
        if (!this.agents[sector]) {
            logError(`Sector no soportado: ${sector}`);
            await whatsappService.sendTextMessage(
                customerData.phone,
                "Lo siento, el tipo de reclamación no está soportado."
            );
        ...
        try {
            ...
            const result = await this.agents[sector].processComplaint(conversation, customerData);
            ...
            logError('Error en el proceso de queja:', {
                error: error.message,
                sector,
                customerId: customerData.phone
            });
